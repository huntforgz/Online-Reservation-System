 ...
 * @author wzz
 * Takes departure airport, arrival airport, departure time and existing airports/airplanes to search.
 * Using caches to store intermediate results in order to make search faster.
...
public class Search {
    ...
    private void tripSearch(int index, ArrayList<Flight> previousFlights, String departureCode, String departureDate) throws ParseException {
        ...
        for (int i = 0; i < caches.size(); i++) {
            if (caches.get(i).getCode().equals(departureCode)) {
                cacheIndex = i;
                break;
            }
        }
        ...
        if (cacheIndex < 0) {
            ...
            if (!departureCode.equals(this.depAirportCode))
                cache.addFlights(HttpUtil.INSTANCE.getFlights(true, this.depTimeToNextDate(), departureCode));
            caches.add(cache);
